...
async def tutor_response(user_input: str, update: Update, profile: dict):
    try:
        ...
        messages = [
            ...
            {"role": "user", "content": f"Please start an English lesson using the topic '{user_topics[user_id]}'. Start by introducing 5 to 10 vocabulary words in {profile['target']} with translations in {profile['native']}. Then, ask the learner to repeat each word twice. After that, listen and provide pronunciation feedback. Only after the vocabulary session, move on to example sentences for practice. Maintain all explanations in {profile['native']} and examples in {profile['target']}."}
        ] + history
