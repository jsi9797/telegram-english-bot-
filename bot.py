...
async def tutor_response(user_input: str, update: Update, profile: dict, mode: str = None):
    try:
        ...
        if mode == "pronunciation":
            ...
            messages.append({"role": "user", "content": f"Please start an English lesson using the topic '{user_topics[user_id]}'. First, introduce 5 to 10 vocabulary words in {profile['target']} with translations in {profile['native']}. After listing the vocabulary, say: '각 단어를 읽어보시고 준비가 되면 녹음하여 전송 해주세요.' Do not include additional instructions or explanations. Then, after vocabulary is completed, provide 3 to 5 example sentences related to the topic. For each sentence: 1) Present the English version, 2) Translate it into {profile['native']}, and 3) Ask the learner to repeat the sentence aloud. Only proceed to the next sentence after that. Maintain all explanations in {profile['native']} and examples in {profile['target']}."})
        messages += history
